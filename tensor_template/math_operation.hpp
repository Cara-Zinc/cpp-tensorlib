...
namespace ts
{
    ...
    inline std::vector<size_t> calculateBroadcastShape(const std::vector<size_t> &shapeA, const std::vector<size_t> &shapeB)
    {
    ...
    inline size_t calculateBroadcastIndex(size_t index, const std::vector<size_t> &shape, const std::vector<size_t> &stride)
    {
    ...
    template <typename T>
    Tensor<T> add(const Tensor<T> &a, double value)
    {
    ...
    template <typename T>
    Tensor<T> Tensor<T>::div(const Tensor<T> &other) const
    {
        return *this / other; // Reuse the operator+ for Tensor objects
    }
